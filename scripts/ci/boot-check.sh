...
#
# WHY THIS IS A FILE AND NOT INLINE YAML: the action splits a multi-line `script:`
# input LINE-BY-LINE and runs every line as its own `sh -c '<line>'` process, using
# /usr/bin/sh (dash on ubuntu runners). Shell functions, if/while blocks and `exit 1`
# inside an if-body are therefore shredded, and bashisms abort it immediately:
# the inline version began with `set -euo pipefail`, which dash rejects with
# "set: Illegal option -o pipefail" (exit 2) — so the gate never installed or launched
# the APK, and `continue-on-error: true` made that dead check look green. Any real
# logic MUST live in a committed script like this one; the YAML may only contain the
# single command line, and the step is a hard gate (no continue-on-error).
#
