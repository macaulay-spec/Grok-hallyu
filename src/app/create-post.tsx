...
import { DEMO_MODE, demoStore, makeDemoProfile } from '@/lib/demo';
...
export default function CreatePostScreen() {
  ...
  const handlePost = async () => {
    ...
    if (DEMO_MODE) {
      // Offline demo: add to the in-memory feed; home refetches on focus.
      const author = profile ?? makeDemoProfile();
      const now = new Date().toISOString();
      demoStore.addPost({
        id: `demo-${Date.now()}`,
        user_id: author.id,
        body: body.trim() || null,
        media_urls: mediaUri ? [mediaUri] : null,
        media_type: mediaUri ? 'image' : 'none',
        created_at: now,
        updated_at: now,
        profile: author,
        dramas: [],
