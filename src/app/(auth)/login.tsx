...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    if (DEMO_MODE) {
      // Demo mode: any credentials work, nothing leaves the device.
      setLoading(true);
      const handle = email.trim().split('@')[0] || 'demo';
      await startDemoSession({
        display_name: handle,
        username: handle.toLowerCase().replace(/[^a-z0-9_]/g, '_').slice(0, 30) || 'demo_user',
      });
      setLoading(false);
      router.replace('/');
      return;
      ...
      >
        ...
        >
          ...
          {DEMO_MODE && (
            ...
            >
              Demo mode — any email &amp; password works. Nothing leaves your device.
            </Text>
          )}
        </ScrollView>
      </KeyboardAvoidingView>
