 ...
 *
 * Video bytes live on a dedicated storage project (`VIDEO_STORAGE_URL` — the Rork cloud backend),
 * NOT on the Hallyu app database. The Hallyu database only keeps the ledger (`media_uploads`) and
 * enforces identity + quotas. Flow:
 *
...
export async function uploadVideo(asset: { uri: string; duration?: number; width?: number; height?: number }, postId: string): Promise<{ key: string }> {
  ...
  const put = await FileSystem.uploadAsync(`${VIDEO_STORAGE_URL}/storage/v1/object/upload/sign/${path}?token=${encodeURIComponent(token)}`, asset.uri, {
    httpMethod: 'PUT',
